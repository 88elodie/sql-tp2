...
function user_model_insert($request){
    ...
    $sql = "INSERT INTO user (name, email, DOB, username, user_password) VALUES ('$name','$email','$birthday','$username','$password')";
...
function user_model_edit($request){
    ...
    $sql = "UPDATE user SET name = '$name', email = '$email', DOB = '$birthday' WHERE user_id = '$userId'";
    mysqli_query($con, $sql);
    mysqli_close($con);
}
...
function user_model_verification($request){
    ...
    if (empty($password) || empty($name) || empty($email) || empty($username) || empty($birthday)){
